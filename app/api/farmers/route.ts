  ...
  try {
    ...
    async function uploadFile(file: File | null, folder: string) {
      ...
      if (error) {
        ...
        throw new Error(
          `Failed to upload ${folder} document: ${error.message}`
        );
      }
    ...
    const fieldDocs = await Promise.all(
      validatedData.fields.map((_, index) =>
        uploadFile(formData.get(`fieldDoc_${index}`) as File, "land-doc")
      )
    );
    ...
    if (
      ...
      (error.message.includes("document is required") ||
      ...
      return NextResponse.json(
        {
          error:
            errorMessages[
              supabaseError.statusCode as keyof typeof errorMessages
            ] || errorMessages.default,
          detail: supabaseError.message,
        },
        {
          status:
            supabaseError.statusCode === "413" ||
            supabaseError.statusCode === "400"
              ? 400
              : 500,
        }
      );
    ...
    if (prismaError.code) {
      ...
      const errorInfo =
        prismaErrors[prismaError.code as keyof typeof prismaErrors];
      if (errorInfo) {
...
export async function GET(request: Request) {
  try {
    ...
    console.log("Query params:", { page, limit, name, state, surveyNumber });
    ...
    if (surveyNumber) {
      where.surveyNumber = { contains: surveyNumber, mode: "insensitive" };
    }
