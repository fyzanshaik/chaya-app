...
export function FarmerForm({ onSuccess }: { onSuccess?: () => void }) {
  const handleLocationUpdate =
    (index: number) => (locationData: LocationData) => {
              ...
              <div key={field.id} className="space-y-4 mb-6">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="col-span-2 space-y-2">
                    ...
                    <LocationButton
                      onLocationUpdate={handleLocationUpdate(index)}
                      fieldIndex={index}
                    />
                    ...
                    {form.watch(`fields.${index}.location`) && (
                      <div className="mt-2 p-3 bg-gray-50 rounded-md space-y-1">
                        <p className="text-sm text-gray-600">
                          Latitude:{" "}
                          {form
                            .watch(`fields.${index}.location.lat`)
                            .toFixed(6)}
                          °N
                        </p>
                        <p className="text-sm text-gray-600">
                          Longitude:{" "}
                          {form
                            .watch(`fields.${index}.location.lng`)
                            .toFixed(6)}
                          °E
                        </p>
                        <p className="text-sm text-gray-600">
                          Accuracy: ±
                          {form
                            .watch(`fields.${index}.location.accuracy`)
                            .toFixed(1)}
                          m
                        </p>
                        {form.watch(`fields.${index}.location.altitude`) && (
                          <p className="text-sm text-gray-600">
                            Altitude:{" "}
                            {form.watch(`fields.${index}.location.altitude`)}m
                            {form.watch(
                              `fields.${index}.location.altitudeAccuracy`
                            ) &&
                              ` (±${form.watch(
                                `fields.${index}.location.altitudeAccuracy`
                              )}m)`}
                        ...
                        <p className="text-sm text-gray-600">
                          Captured:{" "}
                          {new Date(
                            form.watch(`fields.${index}.location.timestamp`)
                          ).toLocaleString()}
                        </p>
                      </div>
                    )}
